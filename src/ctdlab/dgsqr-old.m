...
%if abs(cmax) < tol
if norm(A(2:m,2:n)) <= tol % termination cond from Golub and VanLoan
    k = 1;
    Q = Q(:,k);
    R = R(k,:);
...
for k = 2:maxrnk
    ...
    % find next pivot
    if k < maxrnk
        c((k+1):n) = sqrt(sum(A((k+1):m,(k+1):n).^2,1)); % Slower, but works
        %c((k+1):n) = c((k+1):n) - R(k,(k+1):n).^2;
    ...
    % check for convergence
    %if abs(cmax) < tol
    if norm(A((k+1):m,(k+1):n)) <=tol % termination cond from Golub and VanLoan
        Q = Q(:,k);
        R = R(k,:);
