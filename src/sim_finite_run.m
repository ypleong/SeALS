...
[lambda,grid,R,noise_cov,F_all,uref,D,fFunc,GFunc,BFunc,qFunc,bdim,bcon,region] = deal(sim_data{:});
...
for m = 1:n_trial
    ...
    for k = 1:t_length-1
%         Fiter = mfile.F_all(1,t_length-k);
        Fiter = F_all(1,t_length-k);
        dF = cell(d,1);
        for i = 1:d
            dF{i} = SRMultV(D{i},Fiter{1});
        end
        ...
        c_G = GFunc(current);
        c_B = BFunc(current);
        c_F = EvalT(Fiter{1},current,grid);
...
%     end
    plot( t(1:end_list(j)-1), squeeze( traj_u(:,1:end_list(j)-1,i) ),'linewidth',1.5);
    hold off
    xlabel('Time(s)')
    ylabel(['u_',num2str(i)])
    box on
...
c_tot = zeros(n_trial,1);
for j = 1:n_trial
    c_tot(j) = trapz(t(1:end_list(j)-1),c_traj(j,1:end_list(j)-1));
end
